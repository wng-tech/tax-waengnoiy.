...
// ลิงก์ "เผยแพร่สู่เว็บ" (Publish to web → output:csv) ของแต่ละตาราง
// วิธีขอลิงก์นี้: เปิด Google Sheet > File > Share > Publish to web > เลือกแท็บที่ต้องการ > format: CSV > Publish

const LT_PRICE_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQYZpkIZONDriKxtd29DnZbhMiRi-lZ14UczQ255n_L8lFqXRchMVhRZdjLGX7A1t4ZqrX0TvXHOt7c/pub?output=csv";
const LT_DEP_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vTqjaHfpFtOdHm90TQqtDsD4mm768UKhg-tXnOVV--dyF00U_6UxrcZ1nwPs0RQWuYXqPssMkxWXc5N/pub?output=csv";

function ltPriceSheetUrl() {
  return LT_PRICE_CSV_URL;
}
function ltDepSheetUrl() {
  return LT_DEP_CSV_URL;
}

// เติมเครื่องหมาย % ให้ค่าที่เป็นตัวเลขล้วน (แถวที่ยังไม่ถึงเพดานค่าเสื่อม จะถูกบันทึกในชีตเป็นตัวเลขล้วนไม่มี %)
function ltEnsurePercent(text) {
  const t = (text || "").trim();
...
async function ltLoadFromSheet() {
  ...
  try {
    ...
    const newPrices = rows.map(r => ({ code: r[0], name: r[1], price: parseFloat((r[2] || "0").replace(/,/g, "")) || 0 })).filter(r => r.name);
  ...
  try {
    ...
    if (!res.ok) throw new Error("HTTP " + res.status);
    const rows = ltParseCsv(await res.text()).slice(1).filter(r => r[0]);
    const newDep = rows.map(r => ({ age: parseInt(r[0], 10), tuek: ltEnsurePercent(r[1]), half: ltEnsurePercent(r[2]), wood: ltEnsurePercent(r[3]) })).filter(r => !isNaN(r.age));
